...
object Server extends Parsing:
  ...
  val urlCB: HttpDataCB = CFuncPtr3.fromScalaFunction[Ptr[Parser], CString, Long, Int]:
    (p: Ptr[Parser], data: CString, len: Long) => onURL(p, data, len)

  val onKeyCB: HttpDataCB = CFuncPtr3.fromScalaFunction[Ptr[Parser], CString, Long, Int]:
  ...
  val onBodyCB: HttpDataCB = CFuncPtr3.fromScalaFunction[Ptr[Parser], CString, Long, Int]:
    (p: Ptr[Parser], data: CString, len: Long) => onBody(p, data, len)

  val completeCB: HttpCB = CFuncPtr1.fromScalaFunction[Ptr[Parser], Int]:
    (p: Ptr[Parser]) => onMessageComplete(p)

  val parserSettings = malloc(sizeof[ParserSettings]).asInstanceOf[Ptr[ParserSettings]]
  ...
  val connectCB = CFuncPtr2.fromScalaFunction[TCPHandle, Int, Unit]:
    (server: TCPHandle, status: Int) =>
  ...
  val allocCB = CFuncPtr3.fromScalaFunction[TCPHandle, CSize, Ptr[Buffer], Unit]:
    (handle: TCPHandle, size: CSize, buffer: Ptr[Buffer]) =>
      ...
      buf(4095) = 0.toByte // null termination?
      buffer._1 = buf
