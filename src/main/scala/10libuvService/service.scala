...
def libuvService(args: String*): Unit =
  Service()
    ...
    .get("/")(r => OK(Map("message" -> s"got (routed) request $r")))
    .run(9999)
  uv_run(EventLoop.loop, UV_RUN_DEFAULT)
  println("done")

object ServiceHelpers:
  def OK[T](body: T, headers: Map[String, String] = Map.empty)(using
      e: EncodeJson[T]
  ): Response[String] = Response(200, "OK", headers, body.asJson.nospaces)

case class Service(routes: Seq[Route] = Seq.empty)(using
    ec: ExecutionContext
):
  ...
  def getAsync(
      path: String
  )(h: Request[String] => Future[Response[String]]): Service =
